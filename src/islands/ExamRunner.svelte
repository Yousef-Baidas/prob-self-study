<script lang="ts">
  ...
  import { parseExamSpec, buildExamSession } from '../modes/exam';
  import type { ExamSession } from '../modes/types';
...
{:else if session}
  <section class="exam-run" aria-live="polite">
    ...
    <nav class="pager">
      ...
      {#if atLast}
        <button type="button" class="submit" disabled>Submit</button> <!-- wired in Task 8 -->
      {:else}
